...
class NewScenarioCutIn(ROSBasicScenario):
    """
    ...
    def spawn_vehicle_ahead(self, reference_vehicle, distance):
        ...
        right_vector = spawn_transform.get_right_vector()

        # Calculate the new spawn location
        new_location = spawn_transform.location + right_vector * distance
...
class SpawnAndDriveVehicle(py_trees.composites.Sequence):
    ...
    def initialise(self):
        self.add_child(TimeOfWaitComparison(duration_time = 1))
        ...
        just_drive.add_child(WaypointFollower(self.vehicle, 90 / 3.6,avoid_collision=True)) 
        #self.add_child(AccelerateToCatchUp(self.vehicle, self.ego_vehicles[3], throttle_value=1,
        #                                 delta_velocity=10, trigger_distance=5, max_distance=500))
        just_drive.add_child(InTriggerDistanceToVehicle(
            self.vehicle, self.ego_vehicle, 30))
        self.add_child(just_drive)
        self.add_child(AccelerateToCatchUp(self.vehicle, self.ego_vehicle, throttle_value=1,
                                         delta_velocity=9, trigger_distance=5, max_distance=500))
        self.add_child(LaneChange(self.vehicle, speed=19.44, direction='left', distance_same_lane=5, distance_other_lane=100))   
        self.add_child(LaneChange(self.vehicle, speed=19.44, direction='right', distance_same_lane=5, distance_other_lane=100))                                 
